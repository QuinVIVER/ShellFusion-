...
from utils import readTxt, writeXlsx, readXlsx, writeJson, readJson
        ...
        for line in readXlsx(posts_dir + '/shell_questions(' + site + ').xlsx')[1:]:
            ...
            if accansid != '':
                QAPairs[qid] = {
                    ...
                    'AcceptedAnswer': { 'Id': accansid, 'Body': '' }
                }
...
def extractScripts(abody):
    ...
    for lcp in long_code_patterns:
        ...
        for item in items:
            ...
            ind_script_dict[abody.find(script)] = 'L:' + script  # long script
    ...
    for item in items:
        ...
        if ind not in ind_script_dict:
            ind_script_dict[ind] = 'S:' + script  # short script

    return ind_script_dict

"""
The following program should be performed after completing the steps in 'mp_parser.py' and 'mp_analyzer.py'.
"""

def detectCmdsOpsInQAPairs(posts_dir, mpcmd_info_json, res_json):
    """
    ...
    """
    mpcmd_info_dict, QAPairs_det = readJson(mpcmd_info_json), {}
    ...
    writeJson(QAPairs_det, res_json)


def detectCmdsOpsInGraphScript(script, mpcmd_info_dict):
    """
    Detect cmds and options in a script.
    NOTE: used when generating knowledge graph.
    """
...
def cleanAnswerBody(abody):
    ...
    explanations about cmd's options.
    """
    for lcp in long_code_patterns:
        items = re.finditer(lcp, abody)
        for item in items:
...
if __name__ == '__main__':
    ...
    detectCmdsOpsInQAPairs(conf.exp_posts_dir, _mpcmd_info_json, _QAPairs_det_json)
