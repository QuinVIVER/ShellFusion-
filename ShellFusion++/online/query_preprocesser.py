...
from post_preprocesser import porter_stemmer

eng_stopwords = set()
for sw in stopwords.words('english'):
    eng_stopwords.add(sw)

def preprocess_codematcher(s):
    """
    ...
    return_dict = {v:k for k,v in token_stem_dict.items()}
    return return_dict
def preprocess(_s):
    """
    Preprocess a string, e.g., a query.
    """
    stems, token_stem_dict = [], {}
    s = re.sub('[^a-zA-Z0-9-_+.]', ' ', _s.replace('\n', ' '))
    s = re.sub(' +', ' ', s.lower())
